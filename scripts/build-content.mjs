...
const courseStructure = JSON.parse(fs.readFileSync(path.join(root, "course-structure.json"), "utf8"));
...
const articles = articleIndex.map((metadata) => {
  ...
  return {
    ...
    body: markdown
      ...
      .replace(/]\((?:\.\.\/)*index\.html\)/g, "](#/home)")
  };
});

const output =
  `window.COURSE_STRUCTURE = ${JSON.stringify(courseStructure, null, 2)};\n` +
